...
??? abstract "Quickstart code"
    ...
    best_result = complete_result[list(complete_result.keys())[0]][
        0
    ]  # 0->The first element is the best
...
To overcome these disadvantages, COBRA-k provides an efficient and often quite quick evolutionary algorithm [[Wikipedia]](https://en.wikipedia.org/wiki/Evolutionary_algorithm). This evolutionary algorithm allows one to quite quickly optimize a COBRA-k Model with a full integration of non-linear thermodynamic and kinetic constraints.
...
- An *objective target*, which can be - as for linear optimizations (see LP chapter) - either a single model variable ID (```str```) or a linear term of multiple model variables (```dict[str, float]```). As with all other optimizations, the objective target can be any kind of model variable, including fluxes, concentrations, κ and γ values etc.
...
Again, as we use the NLP, we have to run the preparatory variability analysis as explained in the previous chapter. In COBRA-k, you do not have to call this variability analysis separately in your code as such a variability analysis is automatically calculated if you do not provide already existing variability data.
...
6. Exception step: If any of the previous optimization results in infeasibility (i.e. no solution can be found with the given set of deactivated reactions), we return an appropriate signal to let us know that.
...
$\mathbf{β}$ is optimized through a genetic algorithm [[Wikipedia]](https://en.wikipedia.org/wiki/Genetic_algorithm). This algorithm works in *rounds* and has a given *population (or candidate) size*:
...
1. The first ecTFBA maximizes our objective.
2. The second ecTFBA maximized the number of thermodynamically feasible active reactions under our previously calculated objective value. This allows to find solutions which need more than a minimal or near-minimal set of active reactions.
3. The NLP is then run on all active reactions of the last ecTFBA and returns a fitness value :-)

If any of these three steps shows that there is an infeasibility, an infeasibility signal is sent to the outer optimization.
